...
for i in `ls $resdir/avg_cpu_[0-9]*.dat`;do
    ...
    cp $resdir/perf__${cpunum}_l1-hits $resdir/l1_hits_${cpunum}.dat
    awk -f avg.awk -v count=$iterations \
        $resdir/l1_hits_${cpunum}.dat > $resdir/l1_avg_hits_${cpunum}.dat
   
    #L2 Hits
    cp $resdir/perf__${cpunum}_l2-hits $resdir/l2_hits_${cpunum}.dat
    awk -f avg.awk -v count=$iterations \
    ...
    awk -v maxr=$(wc -l < $resdir/cpu_${cpunum}_result.dat) \
        ...
        $resdir/perf__${cpunum}_tot-access $resdir/perf__${cpunum}_l1-hits \
        > $resdir/l1_misses_${cpunum}.dat

    awk -v maxr=$(wc -l < $resdir/cpu_${cpunum}_result.dat) \
        'NR<=maxr{suba[NR]=$2};NR>maxr{print $1,suba[NR-maxr]-$2}' \
        $resdir/l1_misses_${cpunum}.dat $resdir/perf__${cpunum}_l2-hits \
        > $resdir/l2_misses_${cpunum}.dat

    awk -v maxr=$(wc -l < $resdir/cpu_${cpunum}_result.dat) \
        'NR<=maxr{suba[NR]=$2};NR>maxr{print $1,suba[NR-maxr]-$2}' \
        $resdir/l2_misses_${cpunum}.dat $resdir/perf__${cpunum}_l3-misses \
    ...
    awk -v maxr=$(wc -l < $resdir/cpu_${cpunum}_result.dat) \
        'NR<=maxr{suba[NR]=$2};NR>maxr{print $1,$2/suba[NR-maxr]}' \
        $resdir/perf__${cpunum}_tot-access $resdir/perf__${cpunum}_l3-misses \
